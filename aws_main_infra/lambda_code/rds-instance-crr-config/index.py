...
def handler(event, context):
  ...
  try:
    ...
    try:
      snapshot_complete_waiter = client.get_waiter('db_snapshot_available')
      snapshot_complete_waiter.wait(DBSnapshotIdentifier=snapshotId)
    except botocore.exceptions.WaiterError as e:
      print(e)
    response = drclient.copy_db_snapshot(
      SourceDBSnapshotIdentifier='arn:aws:rds:' + region_name + ':' + aws_account_id + ':snapshot:' + snapshotId,
      TargetDBSnapshotIdentifier='dr-' + snapshotId,
      KmsKeyId=kmsKeyId,
      CopyTags=False,
      Tags=[
          {
